...
const NavItem: React.FC<{
    ...
    <button 
        ...
        ${active ? 'bg-blue-600/10 border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-white hover:bg-white/5'}`}
    >
        <div className={`${active ? 'text-blue-500' : 'text-current'}`}>
            {icon}
        </div>
        <span>{label}</span>
    </button>
...
export const Layout: React.FC<LayoutProps> = ({ currentView, setCurrentView, children }) => {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-[#0b1121] text-white">
            ...
            {/* Sidebar */}
            <aside className={`fixed md:relative z-50 w-64 h-full glass-panel border-r-0 md:border-r border-white/5 flex flex-col transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                <div className="p-6 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center shadow-lg shadow-blue-500/20">
                        <span className="font-bold text-lg text-white">W</span>
                    </div>
                    <span className="text-xl font-bold tracking-tight text-white">Wynxsmapp <span className="text-blue-500">Admin</span></span>
                </div>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative z-0">
                ...
                {/* Top Header */}
                <header className="h-20 flex items-center justify-between px-6 md:px-8 border-b border-white/5 z-10 bg-[#0b1121]/50 backdrop-blur-md">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden md:flex items-center bg-slate-800/50 rounded-full px-4 py-2 border border-white/5 focus-within:border-blue-500/50 transition-colors">
                            <Search size={16} className="text-slate-400 mr-2" />
                            <input type="text" placeholder="Search..." className="bg-transparent border-none focus:outline-none text-sm text-white w-40 placeholder-slate-500" />
                        </div>
                        
                        <div className="flex items-center gap-4">
                            <button className="text-slate-400 hover:text-white relative p-2 hover:bg-white/5 rounded-full transition-colors">
                                <Bell size={20} />
                            ...
                            <div className="flex items-center gap-3 pl-4 border-l border-white/10">
                                <div className="text-right hidden sm:block">
                                    <p className="text-sm font-bold text-white">Mark Johnson</p>
                                    <p className="text-xs text-slate-400">Super Admin</p>
                                </div>
                                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-400 to-indigo-500 p-[2px] shadow-lg shadow-blue-500/20">
                                    <img 
                                        src="https://picsum.photos/id/64/100/100" 
                                        alt="Profile" 
                                        className="w-full h-full rounded-full object-cover border-2 border-[#0b1121]"
                                    />
                                </div>
                            </div>
                        </div>
